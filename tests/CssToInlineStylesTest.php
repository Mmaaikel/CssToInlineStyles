...
namespace mmaaikel\CssToInlineStyles\tests;
 ...
 * @internal
 */
final class CssToInlineStylesTest extends \PHPUnit\Framework\TestCase
{
    /**
    ...
    public function testSimpleElementSelectorV2()
    {
        ...
        $css = 'div { display: none; }';
        $expected = '<h1></h1>' . "\n" . '<div mc:edit="body_content" style="display: block;"><span>test</span></div>';
        $this->runHTMLToCSS($html, $css, $expected);

        $html = '<h1></h1><div mc:edit="body_content" style=""><span>test</span></div>';
        $css = 'div { display: none; }';
        $expected = '<h1></h1>' . "\n" . '<div mc:edit="body_content" style="display: none;"><span>test</span></div>';
        $this->runHTMLToCSS($html, $css, $expected);
...
<a class="one" id="ONE" style="border: 1px solid red; width: 20px !important; border-bottom: 2px; height: 20px; margin: 10px; padding: 100px;">
  ...
  <img class="three" id="THREE" style="padding-bottom: 20px; border: none; padding: 100px; padding-left: 10px;">
  <img class="four" id="FOUR" style="padding-bottom: 20px; padding: 0; border: none; margin: 10px; margin-left: 100px;">
  <img class="five" id="FIVE" style="padding-bottom: 20px; border: none; padding-left: 10px; padding: 100px;">
</a>
EOF;
        $this->runHTMLToCSS($html, $css, $expected);
    }
...
EOF;
    ...
    public function testCleanupWithoutStyleTagCleanup()
    {
        ...
        $css = ' #id { display: inline; } .className { margin-right: 10px; }';
        $expected = '<style>div { top: 1em; }</style>' . "\n" . '<style>div { left: 1em; }</style>' . "\n\n" . '<div style="top: 1em; left: 1em; margin-right: 10px; display: inline;"> id="foo" class="bar" </div>';
        $this->cssToInlineStyles->setUseInlineStylesBlock(true);
        $this->cssToInlineStyles->setStripOriginalStyleTags(false);
    ...
    public function testCleanupWithStyleTagCleanup()
    {
        ...
        $css = ' #id { display: inline; } .className { margin-right: 10px; }';
        $expected = '<style>.cleanup { top: 1em; } div { left: 1em; }</style>' . "\n" . '<div style="left: 1em; margin-right: 10px; display: inline;"> id="foo" class="bar" </div>';
        $this->cssToInlineStyles->setUseInlineStylesBlock(true);
        $this->cssToInlineStyles->setStripOriginalStyleTags(false);
        $this->cssToInlineStyles->setCleanup(true);
        $this->cssToInlineStyles->setHTML($html);
...
background-image: url(\'data:image/jpg;base64,/9j/4QAYRXhpZgAASUkqAAgAA//Z\'); } </style><div class="test"><h1>foo</h1><h1>foo2</h1><table class="bg"></table></div></body></html>';
    ...
    public function testBug99()
    {
        ...
        $css = 'p { color: "red" }';
        $expected = '<p style="color: \'red\';">This is a test heading with various things such as <strong>bold</strong>, <span style="text-decoration: underline;"><strong>bold-underline</strong></span><strong></strong>, <em>italics</em>, and various other cool things.</p>' . "\n" . '<p style="color: \'red\';">Штампы гіст Эйн тэст!</p>';
        $this->cssToInlineStyles->setUseInlineStylesBlock(true);
        $this->cssToInlineStyles->setStripOriginalStyleTags(true);
        $this->cssToInlineStyles->setExcludeMediaQueries(true);
        $this->runHTMLToCSS($html, $css, $expected);
...
EXPECTED;
        ...
        static::assertSame($this->normalizeString($expected), $this->normalizeString($result));
    }
     ...
     * @param bool   $asXHTML
     *
     * @throws \mmaaikel\CssToInlineStyles\Exception
     *
     * @return string
     */
